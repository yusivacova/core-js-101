...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise(((resolve, reject) => {
    const isBooleon = (typeof isPositiveAnswer === 'boolean');
    if (isPositiveAnswer && isBooleon) resolve('Hooray!!! She said "Yes"!');
    if (!isPositiveAnswer && isBooleon) resolve('Oh no, she said "No".');
    if (!isBooleon) reject(new Error('Wrong parameter is passed! Ask her again.'));
  }));
}
    ...
    for (let i = 0; i < array.length; i += 1) {
      Promise.resolve(array[i])
        .then((item) => {
          ...
          if (i === array.length - 1) {
            const newArr = arr.reduce(action);
            resolve(newArr);
          }
        });
    }
